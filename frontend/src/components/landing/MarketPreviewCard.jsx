...
const SPARKLINE_POINTS = [
  [0, 52], [12, 50], [24, 48], [36, 51], [48, 55], [60, 58], [72, 56], [84, 60], [96, 62], [108, 64],
]

function Sparkline() {
  const w = 108
  const h = 36
  const minY = Math.min(...SPARKLINE_POINTS.map((p) => p[1]))
  const maxY = Math.max(...SPARKLINE_POINTS.map((p) => p[1]))
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ display: 'block', overflow: 'visible' }}>
      ...
      <circle cx={SPARKLINE_POINTS.at(-1)[0]} cy={scaleY(SPARKLINE_POINTS.at(-1)[1])} r="3" fill="#16a34a" />
    </svg>
  )
}
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        ...
        <span style={{ fontSize: 11, color: '#6b7280', display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: '#16a34a', display: 'inline-block', animation: 'pulse 2s infinite' }} />
          Live
        </span>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
        {[
          { label: 'YES', pct: 64, color: '#16a34a' },
          { label: 'NO', pct: 36, color: '#dc2626' },
        ].map(({ label, pct, color }) => (
      ...
      >
        <div
          style={{
            ...
            height: 24,
            borderRadius: '50%',
            backgroundColor: 'rgba(21,30,85,0.1)',
            display: 'flex',
            alignItems: 'center',
